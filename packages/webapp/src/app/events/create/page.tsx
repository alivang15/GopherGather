...
export default function CreateEventPage() {
  ...
  // Fetch user_type (and club_id for club_admin) from public.users
  useEffect(() => {
    const fetchUserType = async () => {
      if (user?.id) {
        ...
        if (fetchErr) {
          console.error("Failed to fetch user_type/club_id:", fetchErr);
          setError(fetchErr.message ?? "Failed to determine user type.");
        }
      } else {
