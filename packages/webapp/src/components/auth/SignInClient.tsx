...
export default function SignInClient() {
  ...
  // If already signed in and came from vibe check, redirect back to event and open modal
  useEffect(() => {
  ...
  // If already signed in, redirect immediately (fallback)
  useEffect(() => {
    if (user) {
      router.push(redirectTo);
    }
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        {isVibeCheckFlow ? (
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            Or{" "}
            <Link href="/auth/sign-up" className="font-medium text-[#7a0019] hover:text-red-800 underline">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white py-8 px-4 shadow-lg sm:rounded-lg sm:px-10 border border-gray-200">
          {isVibeCheckFlow && (
            <div className="mb-6 p-4 bg-purple-50 border border-purple-200 rounded-lg">
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-sm font-medium text-purple-800">Vibe Check Authentication</h3>
                  <p className="text-xs text-purple-600 mt-1">After signing in, you&apos;ll return to the event to share your vibe check</p>
                </div>
              </div>
            </div>
          )}
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            {error && <div className="rounded-md bg-red-50 p-4 border border-red-200 text-red-800 text-sm mb-4">{error}</div>}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <Link href="/auth/forgot-password" className="font-medium text-[#7a0019] hover:text-red-800 underline">Forgot your password?</Link>
              </div>
            </div>
